...
2. Check if the length of `pattern` and the array; if not, return false.
...
1. **Time Complexity**: `O(m+n)` → `O(n)`
    ...
    - Since we traverse both once and `m = n`, this simplifies to `O(n)`

2. **Space Complexity**: `O(m+n)` → `O(n)`
    - `m`: length of the `pattern`
    - `n`: length of the `words`
